module Aradia.Backend.Types.Common where

import Control.Lens

import Data.Aeson
import qualified Data.Aeson.Encoding as AE
import Data.Aeson.TH
import Data.Bits
import Data.Bits.Lens
import Data.Char
import Data.Scientific
import Data.Set (Set)
import qualified Data.Set as S
import Data.Text (Text)
import qualified Data.Text as T
import Data.Text.Read
import Data.Word

import Aradia.Backend.Utils

newtype Nullable a = Nullable (Maybe a)
  deriving ( Show, Eq, Ord, Monoid, Functor
           , Applicative, Monad, Foldable, Traversable )

deriveJSON defaultOptions { unwrapUnaryRecords = True
                          , omitNothingFields = False } ''Nullable

newtype Snowflake a = Snowflake { snowflakeToWord :: Word64 }
  deriving ( Show, Eq, Ord )

instance FromJSON (Snowflake a) where
  parseJSON = withText "Snowflake" $ \t -> case decimal t of
    Left err -> fail err
    Right (x,_) -> pure $ Snowflake x

instance ToJSON (Snowflake a) where
  toJSON = String . T.pack . show . snowflakeToWord
  toEncoding = AE.word64Text . snowflakeToWord

coerceSnowflake :: Snowflake a -> Snowflake b
coerceSnowflake = Snowflake . snowflakeToWord

newtype DiscordImage = DiscordImage Text
  deriving (Show, Eq, Ord)

deriveJSON defaultOptions { unwrapUnaryRecords = True} ''DiscordImage

newtype Color = Color (Word8, Word8, Word8)
  deriving (Show, Eq, Ord)

colorAsIntegral :: (Integral a, Bits a) => Color -> a
colorAsIntegral (Color (r,g,b)) = byteAt 2 .~ r $ byteAt 1 .~ g $ byteAt 0 .~ b $ 0

instance FromJSON Color where
  parseJSON = withScientific "Color" $ \n -> 
    case toBoundedInteger n :: Maybe Word32 of
      Just n -> pure $ Color (n ^. byteAt 2, n ^. byteAt 1, n ^. byteAt 0)
      _ -> fail "Couldn't convert color value to RGB8 tuple"

instance ToJSON Color where
  toJSON = toJSON . colorAsIntegral @Word32
  toEncoding = AE.word32 . colorAsIntegral

mkEnum "Permission" $ asBitfield [discordTypes|
  CREATE_INSTANT_INVITE 0x00000001 Allows creation of instant invites
  KICK_MEMBERS 0x00000002 Allows kicking members
  BAN_MEMBERS 0x00000004 Allows banning members
  ADMINISTRATOR 0x00000008 Allows all permissions and bypasses channel permission overwrites
  MANAGE_CHANNELS 0x00000010 Allows management and editing of channels
  MANAGE_GUILD 0x00000020 Allows management and editing of the guild
  ADD_REACTIONS 0x00000040 Allows for the addition of reactions to messages
  VIEW_AUDIT_LOG 0x00000080 Allows for viewing of audit logs
  READ_MESSAGES 0x00000400 Allows reading messages in a channel. The channel will not appear for users without this permission
  SEND_MESSAGES 0x00000800 Allows for sending messages in a channel
  SEND_TTS_MESSAGES 0x00001000 Allows for sending of /tts messages
  MANAGE_MESSAGES 0x00002000 Allows for deletion of other users messages
  EMBED_LINKS 0x00004000 Links sent by this user will be auto-embedded
  ATTACH_FILES 0x00008000 Allows for uploading images and files
  READ_MESSAGE_HISTORY 0x00010000 Allows for reading of message history
  MENTION_EVERYONE 0x00020000 Allows for using the @everyone tag to notify all users in a channel, and the @here tag to notify all online users in a channel
  USE_EXTERNAL_EMOJIS 0x00040000 Allows the usage of custom emojis from other servers
  CONNECT 0x00100000 Allows for joining of a voice channel
  SPEAK 0x00200000 Allows for speaking in a voice channel
  MUTE_MEMBERS 0x00400000 Allows for muting members in a voice channel
  DEAFEN_MEMBERS 0x00800000 Allows for deafening of members in a voice channel
  MOVE_MEMBERS 0x01000000 Allows for moving of members between voice channels
  USE_VAD 0x02000000 Allows for using voice-activity-detection in a voice channel
  CHANGE_NICKNAME 0x04000000 Allows for modification of own nickname
  MANAGE_NICKNAMES 0x08000000 Allows for modification of other users nicknames
  MANAGE_ROLES 0x10000000 Allows management and editing of roles
  MANAGE_WEBHOOKS 0x20000000 Allows management and editing of webhooks
  MANAGE_EMOJIS 0x40000000 Allows management and editing of emojis
|]

-- some sort of packed EnumSet would prob be better, but all the existing packages suck
newtype Permissions = Permissions { permissionSet :: Set Permission }
  deriving (Show, Eq, Ord)

permissionWord :: Permissions -> Word
permissionWord = foldr ((.|.) . fromIntegral . fromEnum) 0 . permissionSet

instance FromJSON Permissions where
  parseJSON = withScientific "Permissions" $ \n ->
    case toBoundedInteger n :: Maybe Word of
      Just n -> pure . Permissions $ ifoldMapOf bits (\idx isSet ->
        if isSet then S.singleton (toEnum idx) else mempty) n
      _ -> fail "Expected a Word"

instance ToJSON Permissions where
  toJSON = toJSON . permissionWord
  toEncoding = AE.word . permissionWord

-- |A Discord role
data Role = Role
  { _roleId :: Snowflake Role
  , _roleName :: Text
  , _roleColor :: Color
  , _roleHoist :: Bool
  , _rolePosition :: Int
  , _rolePermissions :: Permissions
  , _roleManaged :: Bool
  , _roleMentionable :: Bool
  } deriving (Show, Eq)

makeLensesWith fixedTypeFields ''Role
deriveApiFieldJSON defaultOptions ''Role

-- |A Discord application (e.g. bot). Empty data type for use with `Snowflake'
data Application
